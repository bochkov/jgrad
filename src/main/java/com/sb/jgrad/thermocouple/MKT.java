...
public final class MKT extends Thermocouple {
    ...
    public String description() {
        return "Номинальная статическая характеристика преобразования термопары типа ТМК(T) { медь/медь-никель (медь/константан) }";
    }
    ...
    public double value(double temp) throws OutOfBoundException {
        ...
        if (temp >= -270 && temp < 0)
            ...
            throw new OutOfBoundException("temp is out of bounds [-270..400]");
    ...
    public double temp(double v) throws OutOfBoundException {
        ...
        if (v >= -5.603 && v < 0)
            ...
            throw new OutOfBoundException("value is out of bounds [-5.603..20.872]");
