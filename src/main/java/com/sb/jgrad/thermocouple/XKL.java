...
public final class XKL extends Thermocouple {
    ...
    public String description() {
        return "Номинальная статическая характеристика преобразования термопары типа ТХК(L) { хромель/копель }";
    }
    ...
    public double value(double temp) throws OutOfBoundException {
        ...
        if (temp >= -200 && temp <= 0)
            ...
            throw new OutOfBoundException("temp is out of bounds [-200..800]");
    ...
    public double temp(double v) throws OutOfBoundException {
        ...
        if (v >= -9.488 && v <= 0.000)
            ...
            throw new OutOfBoundException("value is out of bounds [-9.488..66.466]");
